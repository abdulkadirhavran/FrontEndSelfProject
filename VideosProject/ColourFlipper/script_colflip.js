...
/* Bu kod parçası, hexValues dizisindeki elemanlardan rastgele bir indeks seçerek rastgele bir HEX renk değeri oluşturan `getRandomHexValue()` işlevini içerir.
...
Böylece `getRandomHexValue()` işlevi her çağrıldığında farklı bir HEX renk değeri döndürür. */
...
/* Bu kod parçası, belirli bir uzunlukta rastgele bir HEX dizesi oluşturmak için `getRandomHexValue()` fonksiyon kullanarak `getRandomHexString` işlevini içerir.

İşlevin çalışma mantığı şu şekildedir:

- Bir dize değişkeni olan `hexString` tanımlanır.
- `for` döngüsü, belirtilen uzunluğa (`stringLength`) kadar çalışır.
- Her iterasyonda, `getRandomHexValue()` işlevi çağrılır ve döndürülen rastgele HEX renk değeri `hexString` değişkenine eklenir.
...
newColorButton.addEventListener("click", () => {
  ...
  // document.body.style.backgroundColor = randomHexString;  ŞEKLİNDE DE YAZILABİLİR.
